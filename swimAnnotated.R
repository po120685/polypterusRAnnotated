# ============================================================
#  SWIM MASTER ANALYSIS
# ============================================================
#  What this script does, in plain terms:
#    Fits a mixed-model or permutation-ANOVA analysis (mixed models
#    for repeated measures per fish, permutation ANOVA for variables
#    that don't fit LMM assumptions well, and matching power
#    analyses) across a set of 15 swimming/kinematic variables,
#    reading from and writing to the "swimSend" data folder. Worth
#    knowing up front: the permutation-ANOVA branch (`run_perm()`)
#    runs its own post-hoc test for a significant environment ×
#    training interaction, but — as detailed in that function's
#    comments below — only for that one interaction, not the other
#    two possible interactions; see the notes on that function below
#    for the asymmetry this creates.
#
#    For each of 15 outcome variables, per a manually chosen method
#    ("LMM", "logLMM", or "perm"):
...
#      - Runs post-hoc pairwise/trend comparisons for significant
#        interactions (all three, for LMM/logLMM; only
#        environment:training, for perm — see run_perm() notes).
...
# ============================================================
# (This section header is leftover text from an earlier
# template/version of this file and does not reflect its current
# content — left as-is here to preserve the original source exactly;
# see the file-level comment block above for what this script
# actually does.)
#
...
# POSTHOC RESULTS
# ===========================================================
# Pairwise / trend post-hoc comparisons for significant interaction
# terms, across all variables (LMM/logLMM: up to 3 interactions per
# variable; perm: environment:training only — see run_perm()).
...
df <- read_csv("combinedSwimDataSend.csv")
...
#   - flowSpeed: numeric (continuous predictor, e.g. water flow speed)
df <- df %>%
  mutate(
...
run_lmm <- function(df, response_var) {
  ...
  # -----------------------------
  # STANDARDIZE EFFECT NAMES
  # -----------------------------
  # lmer/lmerTest names dummy-coded terms after their factor level
  # (e.g. "environmentTerrestrial", "trainingTrained"), while the
  # permutation ANOVA (aovperm, in run_perm() below) reports plain
  # factor names (e.g. "environment", "training"). These gsub()
  # calls rewrite the LMM's coefficient names — both main effects
  # and their two-way interactions — to that shared naming
  # convention, so results from both methods can later be
  # compared/filtered by the same `effect` labels.
...
#
# POST-HOC BEHAVIOR: this version runs a post-hoc test when the
# environment:training term is significant. Two things worth
# knowing about that post-hoc step:
#   1. It refits the data with an ordinary linear model (lm()), not
#      a permutation-based model — the permutation ANOVA itself has
#      no direct pairwise-comparison equivalent, so the omnibus
#      significance test is permutation-based but the follow-up
#      pairwise comparison is parametric (via emmeans on the lm()
#      fit). This is a reasonable practical compromise, but it does
#      mean the post-hoc p-values are not on the same permutation
#      basis as the p-value that triggered them.
#   2. Only the environment:training interaction gets this
#      treatment. p_env, p_train, p_env_sp, and p_tr_sp are computed
#      below but only p_env_tr is actually checked/acted on — so
#      environment:flowSpeed and training:flowSpeed interactions are
#      never followed up with post-hoc tests for perm-method
#      variables, even if significant (unlike the LMM branch, which
#      handles all three interactions symmetrically).
# ------------------------------------------------------------
run_perm <- function(df, response_var, n_permutations = 5000) {
  ...
  perm_table <- as.data.frame(perm_summary)
  
  # identify p-value column
  # (prefer the permutation-based "resampled" p-value; fall back to
  # a parametric "Pr(...)"-style column if that's not present)
  p_col <- grep("resampled", colnames(perm_table), value = TRUE)
  if(length(p_col) == 0)
    p_col <- grep("Pr", colnames(perm_table), value = TRUE)
  
  p_col <- p_col[1]
  
  get_p <- function(term){
    
    if(term %in% rownames(perm_table)){
      return(perm_table[term, p_col])
    } else{
      return(NA)
    }
  ...
  # NOTE: p_env and p_train are computed here but never used below —
  # only the three interaction p-values are consulted, and of those,
  # only p_env_tr actually triggers a post-hoc step (see the
  # function-level comment above).
  p_env      <- get_p("environment")
  p_train    <- get_p("training")
  p_env_tr   <- get_p("environment:training")
  p_env_sp   <- get_p("environment:flowSpeed")
  p_tr_sp    <- get_p("training:flowSpeed")
  ...
  # ENVIRONMENT × TRAINING
  # ---------------------------------------
  # Only interaction that gets a post-hoc follow-up in this
  # permutation branch. Refits the data as a plain OLS model
  # (environment * training + flowSpeed, no random effect) purely so
  # emmeans has a model object to compute estimated marginal means
  # and Tukey-adjusted pairwise comparisons of environment within
  # each level of training from.
  ...
  if(!is.na(p_env_tr) && p_env_tr < 0.05){
    ...
    lm_posthoc <- lm(
      as.formula(
        paste(response_var,
              "~ environment * training + flowSpeed")
      ),
      data = df
    )
...
# of the 15 swim outcome variables. These choices were made manually
...
analysis_plan <- data.frame(
  ...
  method = c(
    ...
    "LMM",  # tail frequency
    "LMM",  # fin frequency
    "logLMM",  # tail amplitude
    "perm",  # fin amplitude
    "perm",  #fin effort
    "logLMM",  # tail effort
    "perm",  # total effort
    "perm",  # Reynolds number
    "logLMM",  # Strouhal number
    "perm"  # body angle
  )
)
# ============================================================
